...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
    	final EventUpdate pre = new EventUpdate(true);
    	pre.call();
        super.onUpdate();

        if (this.isRiding())
        {
            this.connection.sendPacket(new CPacketPlayer.Rotation(RotationUtil.currentRotation[0], RotationUtil.currentRotation[1], this.onGround));
        ...
        {
            this.onUpdateWalkingPlayer(pre.sprinting, pre.sneaking, pre.x, pre.y, pre.z, RotationUtil.currentRotation[0], RotationUtil.currentRotation[1], pre.onGround);
            //System.out.println("Updated from entityplayersp");
            // calls the post event update
            new EventUpdate(false).call();
        }
    }
     ...
     */
    public void onUpdateWalkingPlayer(boolean sprinting, boolean sneaking, double x, double y, double z, float yaw, float pitch, boolean ground)
    {
        boolean flag = sprinting;
        ...
        if (this.rotationPitch != RotationUtil.currentRotation[1]) {
	        this.rotationYawHead = RotationUtil.currentRotation[0];
	        this.rotationPitchHead = RotationUtil.currentRotation[1];
        } else {
        	this.rotationPitchHead = RotationUtil.currentRotation[1];
        }
        RotationUtil.serverRotation[0] = yaw;
        RotationUtil.serverRotation[1] = pitch;
    ...
    {
    	if (message.charAt(0) == '.') Mania.getCommandManager().onChat(message);
    	else this.connection.sendPacket(new CPacketChatMessage(message));
    }
	...
	public void setInWeb(boolean b) {
		this.isInWeb = b;
	}
}
