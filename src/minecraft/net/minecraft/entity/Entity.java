...
public abstract class Entity implements ICommandSender
{
    ...
    public void move(double x, double y, double z)
    {
        ...
        else
        {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = this instanceof EntityPlayer && (this.onGround && this.isSneaking()) || EventOutWalking.call(this.onGround && this.isSneaking());
    ...
    {
    	final boolean isPlayer = this == Minecraft.getMinecraft().player;
    	float yaw = this.rotationYaw;
    	if (isPlayer) {
    		final EventStrafe event = new EventStrafe(forward, strafe, yaw, this.rotationPitch);
    		event.call();
    		final float[] rotation = { this.rotationYaw, this.rotationPitch };
    		//RotationUtil.limiteRotation(rotation, 75f); // smooth rotation
    		//RotationUtil.gcdRotation(rotation, Minecraft.getMinecraft().gameSettings.mouseSensitivity); // GCD fix
    		// update current rotation
    		RotationUtil.setRotation(RotationUtil.currentRotation, rotation);
    		/*
    		 * Deal with movement problems
    		 */
    		yaw = rotation[0];
    	}
