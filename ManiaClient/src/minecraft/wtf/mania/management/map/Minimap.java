...
import wtf.mania.management.map.animation.Animation;
...
public class Minimap
{
    ...
    public static int loadBlockColourFromTexture(final IBlockState state, final Block b, final BlockPos pos, final boolean convert) {
        ...
        final int grassColor = b.getMapColor(state, mc.world, pos).colorValue;
    ...
    public boolean applyTransparentBlock(final Chunk bchunk, final Block b, final IBlockState state, final BlockPos globalPos, final BlockPos pos) {
        ...
        if (b instanceof BlockLiquid && b.getLightOpacity(state) != 255 && b.getLightOpacity(state) != 0) {
        ...
        else if (XaeroMinimap.getSettings().blockColours == 0 && (b.getBlockLayer() == BlockRenderLayer.TRANSLUCENT || b instanceof BlockGlass)) {
            ...
            final int i5 = bchunk.getBlockLightOpacity(pos);
            final int color = b.getMapColor(b.getStateFromMeta(i5), mc.world, pos).colorValue;
            red = (color >> 16 & 0xFF);
            green = (color >> 8 & 0xFF);
            blue = (color & 0xFF);
            skip = true;
    ...
    public Block findBlock(final Chunk bchunk, final int insideX, final int insideZ, final int highY, final int lowY) {
        ...
        for (int i = highY; i >= lowY; --i) {
            final Block got = bchunk.getBlockState(insideX, i, insideZ).getBlock();
            if (!(got instanceof BlockAir) && (underair || this.loadingCaving == -1)) {
                if (got.getRenderType(bchunk.getBlockState(insideX, i, insideZ)).ordinal() != -1) {
                    if (got != Blocks.TORCH) {
                        if (got != Blocks.TALLGRASS) {
                            ...
                            if (XaeroMinimap.getSettings().blockColours == 0) {
                            	state = got.getActualState(state, mc.world, globalPos);
                            }
                            if (!this.applyTransparentBlock(bchunk, got, state, globalPos, pos)) {
                                if (XaeroMinimap.getSettings().blockColours == 1) {
                                    final MapColor minimapColor = got.getMapColor(state, mc.world, globalPos);
    ...
    public void loadBlockColor(final int par1, final int par2, final Chunk bchunk, final int chunkX, final int chunkZ) {
        ...
        if (this.lastBlockY[insideX] <= 0) {
            ...
            final Chunk prevChunk = Minimap.mc.world.getChunkFromChunkCoords(this.loadingMapX + chunkX, this.loadingMapZ + chunkZ - 1);
    ...
    private int getCaving() {
        ...
        final Chunk bchunk = Minimap.mc.world.getChunkFromChunkCoords(chunkX, chunkZ);
        final int top = bchunk.getHeightValue(insideX, insideZ);
        if (y < top) {
            for (int i = y; i < top; ++i) {
                if (bchunk.getBlockState(insideX, i, insideZ).getMaterial().isOpaque()) {
                	Math.min(y + 3, i);
                }
            }
        }
