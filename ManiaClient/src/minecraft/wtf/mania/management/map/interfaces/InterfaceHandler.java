package wtf.mania.management.map.interfaces;
...
import wtf.mania.management.map.Minimap;
import wtf.mania.management.map.XaeroMinimap;
import wtf.mania.management.map.settings.ModOptions;
...
public class InterfaceHandler
{
    ...
    public static void load() {
        ...
        InterfaceHandler.minimap = new Minimap(new Interface("gui.xaero_minimap", 128, 128, ModOptions.MINIMAP) {
            ...
            @Override
            public void drawInterface(final int width, final int height, final float scale, final float partial) {
                ...
                if (XaeroMinimap.getSettings().getShowCoords()) {
                    ...
                    String coords = Minimap.myFloor(InterfaceHandler.mc.player.posX) + ", " + Minimap.myFloor(InterfaceHandler.mc.player.posY) + ", " + Minimap.myFloor(InterfaceHandler.mc.player.posZ);
                    if (InterfaceHandler.mc.fontRendererObj.getStringWidth(coords) >= interfaceSize) {
                        final String stringLevel = "" + Minimap.myFloor(InterfaceHandler.mc.player.posY);
                        coords = Minimap.myFloor(InterfaceHandler.mc.player.posX) + ", " + Minimap.myFloor(InterfaceHandler.mc.player.posZ);
                ...
                if (XaeroMinimap.getSettings().showBiome) {
                    ...
                    this.underText.add(InterfaceHandler.mc.world.getChunkFromBlockCoords(pos).getBiome(pos, InterfaceHandler.mc.world.getBiomeProvider()).getBiomeName());
                }
            ...
            private void drawArrow(final double arrowX, final double arrowY, final float[] colour) {
                ...
                GlStateManager.rotate(InterfaceHandler.mc.player.rotationYaw, 0.0f, 0.0f, 1.0f);
