...
public class Projectiles extends Module {
	...
	public Projectiles() {
		super("Projectiles", "Predict the path of a projectile", ModuleCategory.Render, true);
	}
	
	@EventTarget
	public void onRender3D(EventRender3D evnet) {
        ...
        while (!flightPath.isCollided()) {
            ...
            flightPoint.offer(new Vec3d(flightPath.position.xCoord - mc.getRenderManager().viewerPosX,
        ...
        while (!flightPoint.isEmpty()) {
            ...
            bufferbuilder.pos(head.xCoord, head.yCoord, head.zCoord).color(1f, 1f, 1f, 1f).endVertex();

            if (flightPoint.peek() != null) {
                Vec3d point = flightPoint.peek();
                bufferbuilder.pos(point.xCoord, point.yCoord, point.zCoord).color(1f, 1f, 1f, 1f).endVertex();
            }
        ...
        if (flightPath.collided) {
            ...
            if (hit.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                if (iblockstate.getMaterial() != Material.AIR && mc.world.getWorldBorder().contains(blockpos)) {
                    final Vec3d interp = MiscUtils.interpolateEntity(mc.player, mc.getRenderPartialTicks());
                    bb = iblockstate.getSelectedBoundingBox(mc.world, blockpos).contract(0.0020000000949949026D).offset(-interp.xCoord, -interp.yCoord, -interp.zCoord);
                }
            ...
            if (bb != null) {
               // Render3DUtils.drawBoundingBox(bb, width, -1);
            }
        }
        Render3DUtils.end3D();
    }
    ...
    private final class FlightPath {
        ...
        public void onUpdate() {
            ...
            this.motion = MiscUtils.mult(this.motion, motionModifier);
        ...
        private void onCollideWithEntity(Vec3d prediction, RayTraceResult blockCollision) {
            ...
            List<Entity> collisionEntities = mc.world.getEntitiesWithinAABBExcludingEntity(this.shooter, this.boundingBox.expand(this.motion.xCoord, this.motion.yCoord, this.motion.zCoord).addCoord(1.0D, 1.0D, 1.0D));
        ...
        private void setPosition(Vec3d position) {
            this.position = new Vec3d(position.xCoord, position.yCoord, position.zCoord);
        ...
        private void setThrowableHeading(Vec3d motion, float velocity) {
            ...
            this.motion = MiscUtils.div(motion, (float) motion.lengthVector());
            // Multiply by the velocity
            this.motion = MiscUtils.mult(this.motion, velocity);
        }
