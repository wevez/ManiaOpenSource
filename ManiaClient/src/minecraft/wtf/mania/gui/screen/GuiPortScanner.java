...
public class GuiPortScanner extends GuiScreen {
    ...
    private String status = "Åò7Wating...";
    ...
    public void initGui() {
        ...
        hostField = new GuiTextField(0, fontRendererObj, width / 2 - 100, 60, 200, 20);
        hostField.setFocused(true);
        hostField.setMaxStringLength(Integer.MAX_VALUE);
        hostField.setText("localhost");

        minPortField = new GuiTextField(1, fontRendererObj, width / 2 - 100, 90, 90, 20);
        minPortField.setMaxStringLength(5);
        minPortField.setText(String.valueOf(1));

        maxPortField = new GuiTextField(2, fontRendererObj, width / 2 + 10, 90, 90, 20);
        maxPortField.setMaxStringLength(5);
        maxPortField.setText(String.valueOf(65535));

        threadsField = new GuiTextField(3, fontRendererObj, width / 2 - 100, 120, 200, 20);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	mc.getTextureManager().bindTexture(new ResourceLocation("client/back.jpg"));
		this.drawModalRectWithCustomSizedTexture(0, 0, 0, 0, width, height, width, height);
		this.drawGradientRect(0, height / 2, width, height, 0x00000000, 0xff000000);
		this.drawGradientRect(0, 0, width, height / 2, 0xff000000, 0x00000000);


        mc.fontRendererObj.drawString("Port Scanner", width / 2, 34, 0xffffff);
        mc.fontRendererObj.drawString( running ? "Åò7" + checkedPort + " Åò8/ Åò7" + maxPort : status == null ? "" : status, width / 2, height / 4 + 80, 0xffffff);
        ...
        mc.fontRendererObj.drawString("ÅòcÅòlPorts:", 2, 2, Color.WHITE.hashCode());

        synchronized(ports) {
            int i = 12;

            for(final Integer integer : ports) {
                mc.fontRendererObj.drawString(String.valueOf(integer), 2, i, Color.WHITE.hashCode());
        ...
        switch(button.id) {
                    ...
                    for(int i = 0; i < threads; i++) {
                        new Thread(() -> {
                            try {
                                ...
                                status = "ÅòaÅòl" + e.getClass().getSimpleName() + ": Åòc" + e.getMessage();
                            }
                        }).start();
                    }
            ...
            case 2:
                ...
                try {
                    ...
                    fileWriter.write("efsun.cc\r\n");
                    fileWriter.write("Portscan\r\n");
                    fileWriter.write("IP: " + host + "\r\n\r\n");

                    fileWriter.write("Server Ports (" + minPort + " - " + maxPort + "):\r\n");
    ...
    private static void tab(final GuiTextField... textFields) {
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if(Keyboard.KEY_TAB == keyCode) tab(hostField, minPortField, maxPortField);
