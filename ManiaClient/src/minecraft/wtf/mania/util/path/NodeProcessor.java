...
public abstract class NodeProcessor {
	...
	protected ArrayList<Node> getNeighbors(Node node) {
		...
		for(BlockPos pos : BlockPos.getAllInBox(nodeBlockPos.add(1, 1, 1), nodeBlockPos.add(-1, -1, -1))) {
			...
			if(pos.getY() < nodeBlockPos.getY() && !isPassable(PathUtils.getBlockState(pos.up(2)))) {
				continue;
			}
			if(pos.getY() > nodeBlockPos.getY() && !isPassable(PathUtils.getBlockState(nodeBlockPos.up(2)))) {
				continue;
			}
