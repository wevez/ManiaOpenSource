...
public class FlyingNodeProcessor extends NodeProcessor {
	...
	public Node createNode(BlockPos pos) {
		Node node = new Node(!isStairBlock(PathUtils.getBlock(pos.down())) && isPassable(PathUtils.getBlockState(pos)) && isPassable(PathUtils.getBlockState(pos.up())), pos).setId(pos.hashCode());
	...
	public boolean isVclipableDown(Node node) {
		...
		if(isPassable(PathUtils.getBlockState(blockPos.down()))) {
			return false;
		}
		for(int i = 1; i < 9; i++) {
			BlockPos pos = new BlockPos(blockPos.getX(), blockPos.getY() - i, blockPos.getZ());
			if(isPassable(PathUtils.getBlockState(pos)) && i < 8
					&& isPassable(PathUtils.getBlockState(pos.up()))) {
	...
	public boolean isVclipableUp(Node node) {
		...
		BlockPos blockPos = node.getBlockpos();
		if(isPassable(PathUtils.getBlockState(blockPos.up()))) {
			return false;
		}
		for(int i = 1; i < 9; i++) {
			BlockPos pos = new BlockPos(blockPos.getX(), blockPos.getY() + i, blockPos.getZ());
