package wtf.mania.util.sound.player;
...
import wtf.mania.util.sound.Player;
import wtf.mania.util.sound.util.BitstreamException;
import wtf.mania.util.sound.util.Decoder;
import wtf.mania.util.sound.util.DecoderException;
import wtf.mania.util.sound.util.Frame;
import wtf.mania.util.sound.util.SampleBuffer;
import wtf.mania.util.sound.util.SoundStream;
...
public class MP3Player {
	...
	/**
	 * Appends the specified file (or all the files, recursively only if specified,
	 * if represents a folder) to the end of the play list.
	 */
				...
				public void run() {
					...
					try {
						...
						if (playlistObject instanceof File) {
							...
							URLConnection conn = ((URL) playlistObject).openConnection();
							conn.setRequestProperty("User-agent", "Mozilla/5.0");
							inputStream = conn.getInputStream();
							inputStreamVar = conn.getInputStream();
						} else {
							throw new IOException(
									"this is impossible; how come the play list contains this kind of object? :: "
											+ playlistObject.getClass());
						}
	...
	/**
	 * Forces the player to play next mp3 in the play list (or random if shuffle is
	 * turned on).
	 * 
	 * @see #play()
	 */
	...
	/**
	 * Forces the player to play previous mp3 in the play list (or random if shuffle
	 * is turned on).
	 * 
	 * @see #play()
	 */
	...
	/**
	 * Returns the shuffle state of the player. True if the shuffle is on, false if
	 * it's not.
	 * 
	...
	public int getFrameLength() {
		int pos = 0;
		if (playingSource != null) {
			pos = (int) (playingSource.getBufferSize() / 1000);
		}
		return pos;
	}
