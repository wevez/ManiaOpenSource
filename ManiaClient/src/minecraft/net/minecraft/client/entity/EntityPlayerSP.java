...
import wtf.mania.module.impl.movement.NoSlow;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    float[] cdgoRotation = new float[]{0, 0};
    ...
    {
    	EventUpdate event = new EventUpdate(this.posX, this.posY, this.posZ, this.onGround, this.isSprinting(), this.isSneaking());
        event.call();
        
        if (event.isCancelled()) return;
        MatrixTimer.onUpdate();
        RotationUtils.serverRotations[0] = EventRotation.currentRotations[0];
        RotationUtils.serverRotations[1] = EventRotation.currentRotations[1];
		this.rotationYawHead = RotationUtils.serverRotations[0];
        this.rotationPitchHead = RotationUtils.serverRotations[1];
        this.renderYawOffset = RotationUtils.serverRotations[0];
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            double d3 = (double)(EventRotation.currentRotations[0] - this.lastReportedYaw);
            double d4 = (double)(EventRotation.currentRotations[1] - this.lastReportedPitch);
            ...
            if (this.isRiding())
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0D, this.motionZ, EventRotation.currentRotations[0], EventRotation.currentRotations[1], event.onGround));
                flag2 = false;
            }
            else if (flag2 && flag3)
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(event.x, event.y, event.z, EventRotation.currentRotations[0], EventRotation.currentRotations[1], event.onGround));
            }
            else if (flag2)
            {
            ...
            {
                this.connection.sendPacket(new CPacketPlayer.Rotation(EventRotation.currentRotations[0], EventRotation.currentRotations[1], event.onGround));
            }
            ...
            if (flag3)
            {
                this.lastReportedYaw = EventRotation.currentRotations[0];
                this.lastReportedPitch = EventRotation.currentRotations[1];
            }
        ...
        EventUpdate post = new EventUpdate();
        post.call();
    }
    ...
    {
    	if (!Mania.instance.commandManager.onSendMessage(message))
    ...
    public void onLivingUpdate()
    {
        ...
        {
        	if(NoSlow.instance.toggled) {
        		this.movementInput.moveStrafe *= NoSlow.speed.value;
	            this.movementInput.field_192832_b *= NoSlow.speed.value;
        	}else {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        EventMove em = new EventMove((float) p_70091_2_, (float) p_70091_4_, (float) p_70091_6_).call();
        super.moveEntity(x, MathHelper.clamp(em.x, -10, 10), MathHelper.clamp(em.y, -10, 10), MathHelper.clamp(em.z, -10, 10));
        this.updateAutoJump((float) (this.posX - em.x), (float) (this.posZ - em.z));
    }
