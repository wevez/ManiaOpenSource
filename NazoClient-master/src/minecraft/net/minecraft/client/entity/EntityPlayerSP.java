...
import nazo.module.movement.NoSlowDown;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
	...
	{
		EventUpdate event = new EventUpdate(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround, this.isSprinting(), this.isSneaking());
		...
		this.serverPosX = (int) event.x;
		this.serverPosY = (int) event.y;
		this.serverPosZ = (int) event.z;
		...
		if (this.mc.gameSettings.thirdPersonView != 0) {
			this.renderYawOffset = event.yaw;
			//TODO
			//this.rotationpitchhead = event.pitch;
			this.rotationYawHead = event.yaw;
		}
		if (isCurrentViewEntity()) {
			...
			if (this.ridingEntity == null) {
				if (flag2 && flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.x,
							event.y, event.z, event.yaw,
							event.pitch, event.onGround));
				} else if (flag2) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.x,
							event.y, event.z, event.onGround));
				} else if (flag3) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event.yaw,
							event.pitch, event.onGround));
				} else {
				...
				this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
						this.motionZ, this.rotationYaw, this.rotationPitch, event.onGround));
				flag2 = false;
			}
	...
	public void sendChatMessage(String message)
	{
		if(message.charAt(0) == '.') {
			...
			for(Command c : Nazo.commandManager.commands) {
				for(String s : c.name) {
					if(s.equalsIgnoreCase(str[0].substring(1))) {
						c.a(str);
						return;
					}
				}
			}
			ChatUtils.printChatprefix("Command not found");
			ChatUtils.printChatprefix("You can type .help to check commands");
		} else {
