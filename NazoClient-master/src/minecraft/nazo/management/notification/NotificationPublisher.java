...
import nazo.Nazo;
import nazo.module.render.HUD;
import nazo.utils.AnimationUtils;
...
public final class NotificationPublisher {
   ...
   public static void publish(ScaledResolution sr) {
      if (!NOTIFICATIONS.isEmpty()) {
         ...
         HUD hud = (HUD)Nazo.moduleManager.getClass(HUD.class);
         NazoFontRenderer fr = Nazo.mainFont;
         ...
         for(Iterator var7 = NOTIFICATIONS.iterator(); var7.hasNext(); y -= 30) {
            ...
            Gui.drawRect((double)translateX, (double)(translateY + 30.0F - 2.0F), (double)translateX + (int)width * ((double)((long)notification.getTime() - notification.getTimer().getElapsedTime()) / (double)notification.getTime()), (double)(translateY + 30.0F), notification.getType().getColor());
