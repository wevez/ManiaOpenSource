...
public class NazoFontRenderer extends NazoFont {
	protected CharData[] boldChars = new CharData['Åò'];
	protected CharData[] italicChars = new CharData['Åò'];
	protected CharData[] boldItalicChars = new CharData['Åò'];
	...
	public NazoFontRenderer(Font font, boolean antiAlias, boolean fractionalMetrics) {
	...
	public float drawString(String text, double x, double y, int color, final boolean shadow) {
		...
		for (int i = 0; i < size; ++i) {
			...
			if (String.valueOf(character).equals("¬ß")) {
				int colorIndex = 21;
				try {
		...
		if (getStringWidth(text) > width) {
			...
			for (int i = 0; i < j; i++) {
				...
				for (int ii = 0; i < word.toCharArray().length; i++) {
					...
					if ((c == 'Åò') && (i < word.toCharArray().length - 1)) {
				...
				if (getStringWidth(currentWord + word + " ") < width) {
					...
					currentWord = 'Åò' + lastColorCode + word + " ";
				}
			}
			if (currentWord.length() > 0)
				if (getStringWidth(currentWord) < width) {
					finalWords.add('Åò' + lastColorCode + currentWord + " ");
					currentWord = "";
				} else {
	...
	public List<String> formatString(String string, double width) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			if (getStringWidth(currentWord + c) < width) {
				...
				finalWords.add(currentWord);
				currentWord = 'Åò' + lastColorCode + String.valueOf(c);
			}
		}
